...
from models.nextmeta_decoder import nextmeta_decoder
import torch.nn as nn
...
class Bottleneck(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.dwconv     = nn.Conv2d(med_channels, med_channels, kernel_size=3, padding="same", groups=med_channels)
        self.norm       = nn.LayerNorm(med_channels)    
        self.pwconv2    = nn.Linear(med_channels, out_c)
        self.act        = nn.GELU()
               
    def forward(self, inputs):
        x   =   self.pwconv1(inputs.permute(0, 2, 3, 1))
        x   =   self.act(x).permute(0, 3, 1, 2)
        x   =   self.dwconv(x).permute(0, 3, 1, 2)

        convout = self.pwconv2(self.act(x))

        out     = convout+inputs
        return out
#####   MODEL #####
    
class CA_CBA_NEXTM_huge(nn.Module):
    def __init__(self,n_classes,config_res=None,training_mode=None,imnetpretrained=None):
        ...
        self.metanext       = nextmeta_decoder()
