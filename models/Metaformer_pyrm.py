...
class MetaFormer(nn.Module):
    ...
    def get_features(self, x,p_features):
        out=[]
        for i in range(self.num_stage):
            x = self.downsample_layers[i](x)
            x = self.stages[i](x)+p_features[i].permute(0, 2,3,1)
            out_ = x.permute(0, 3, 1, 2)
            out.append(out_)
        return self.norm(x.mean([1, 2])),out

    def forward(self, x,p_features):
        x,features = self.get_features(x,p_features)
        x = self.head(x)
        return x,features


def caformer_s18_in21ft1k_py(config_res,training_mode=None,pretrained=None,**kwargs):
