...
#import time
from models.Metaformer_pyrm import caformer_s18_in21ft1k_py
...
class down_block(nn.Module):
    def __init__(self, in_c, out_c,kernel_size,stride, padding):
        ...
        self.conv_block=nn.Sequential(nn.Conv2d(in_c, out_c, kernel_size=kernel_size, stride=stride ,padding=padding),
...
DOWNSAMPLE_LAYERS_FOUR_STAGES = [partial(down_block,
...
class CA_CBA_Convnext_pyrmd(nn.Module):
    def __init__(self,n_classes,config_res=None,training_mode=None,imnetpretrained=None):
        ...
        if not self.training_mode ==  "ssl_pretrained": 
            self.caformer        = caformer_s18_in21ft1k_py(config_res,training_mode,imnetpretrained)

        self.output_norms = nn.ModuleList([nn.LayerNorm(i) for i in size_dec[::-1]])


        self.downsample_layers  = nn.ModuleList([DOWNSAMPLE_LAYERS_FOUR_STAGES[i](size_en[i], size_en[i+1]) for i in range(len(size_en)-1)])
        downsample_layers       = nn.Sequential(DOWNSAMPLE_LAYERS_FOUR_STAGES[-1](size_en[-2],size_en[-1]))
    ...
    def forward(self, inputs):                      # 1x  3 x 128 x 128
        ...
        p_feature=inputs
        for i in range (4):
            p_feature = self.downsample_layers[i](p_feature)
            p_features.append(p_feature)

        if self.training_mode ==  "ssl_pretrained": 
            out = inputs
        else:
            _,out = self.caformer(inputs,p_features)               # [2, 64, 64, 64]) ([2, 128, 32, 32]) [2, 320, 16, 16]) ([2, 512, 8, 8])
