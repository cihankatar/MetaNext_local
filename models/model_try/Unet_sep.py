...
class conv_block(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.conv_block=nn.Sequential(DepthWiseSeperable(in_c, out_c, stride=1,kernel_size=3, padding=1),
                                      DepthWiseSeperable(out_c, out_c, stride=1,kernel_size=3, padding=1))
...
class up(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.up   = DepthWiseSeperable(in_c, out_c, stride=1,kernel_size=3, padding=1)
...
class UNET_sep(nn.Module):
    def __init__(self,n_classes):
        ...
        if self.n_classes > 1:
            ...
            self.outputs  = nn.SeparableConv2d(64, 2, kernel_size=1, padding=0)
            #self.outputs = softmax_f(self.outputs)
        else:
            self.outputs  = nn.SeparableConv2d(64, 1, kernel_size=1, padding=0)
