...
class Metafusion3(nn.Module):
    def __init__(self,n_classes):
        ...
        self.conv_module = nn.Sequential(nn.Conv2d(1024,512,kernel_size=1,padding='same'),nn.BatchNorm2d(512),nn.ReLU())
        self.fusion_module = nn.Sequential(nn.Conv2d(512,512,kernel_size=3,padding='same'),nn.BatchNorm2d(512),nn.ReLU())
    ...
    def forward(self, inputs):                         # 1x  3 x 128 x 128
        ...
        _,ca_out = self.caformer(inputs)               # [2, 64, 32, 32]) ([2, 128, 16, 16]) [2, 320, 8, 8]) ([2, 512, 4, 4])
        _,conv_out = self.convformer(inputs) 

        merge = self.conv_module(torch.cat((ca_out[3], conv_out[3]),dim=1))
        #merge = self.conv_module(ca_out[3] + conv_out[3])
        b     = self.fusion_module(merge)

        skip_connection=[]

        length = len(self.Convformer_att)
        for i in range (3):
            skip_fusion = self.Convformer_att[i](ca_out[i]+conv_out[i])
            skip_connection.append(skip_fusion)
